...
export default function SetupPage() {
  ...
  return (
    <main className="min-h-screen pt-24 pb-32 px-4 relative bg-background">
      ...
      {/* Top Header */}
      <header className="fixed top-0 left-0 w-full z-50 flex items-center px-6 h-16 bg-white/5 backdrop-blur-md border-b border-white/10 shadow-sm">
      ...
      <div className="max-w-3xl mx-auto flex flex-col gap-8 md:gap-12 relative z-10 w-full">
        ...
        <div className="flex flex-col gap-4 text-center max-w-2xl mx-auto">
          <Shield className="w-16 h-16 text-cyan-400 mx-auto mb-2 animate-pulse" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto w-full">
          {/* Phone & SMS */}
          <div className="glass-panel rounded-2xl p-6 hover:-translate-y-1 transition-all flex flex-col justify-between h-full border border-white/5">
            <div className="flex justify-between items-start mb-6">
              <div className="bg-primary-container/20 p-3 rounded-xl border border-primary/20">
                <Smartphone className="w-6 h-6 text-primary" />
              </div>
              <button
                onClick={() => handleToggle('phone')}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${granted.phone ? 'bg-cyan-500' : 'bg-surface-container-highest'}`}
              >
            ...
            <div>
              <h3 className="text-lg font-bold text-on-surface mb-2">Phone &amp; SMS</h3>
          ...
          <div className="glass-panel rounded-2xl p-6 hover:-translate-y-1 transition-all flex flex-col justify-between h-full border border-white/5">
            <div className="flex justify-between items-start mb-6">
              <div className="bg-secondary-container/20 p-3 rounded-xl border border-secondary/20">
                <Bell className="w-6 h-6 text-cyan-400" />
              </div>
              <button
                onClick={() => handleToggle('notifications')}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${granted.notifications ? 'bg-cyan-500' : 'bg-surface-container-highest'}`}
              >
            ...
            <div>
              <h3 className="text-lg font-bold text-on-surface mb-2">Push Notifications</h3>
              <p className="text-xs text-on-surface-variant leading-relaxed">
          ...
          <div className="glass-panel rounded-2xl p-6 hover:-translate-y-1 transition-all flex flex-col justify-between h-full border border-white/5">
            <div className="flex justify-between items-start mb-6">
              <div className="bg-tertiary-container/20 p-3 rounded-xl border border-tertiary/20">
                <Eye className="w-6 h-6 text-indigo-400" />
              </div>
              <button
                onClick={() => handleToggle('accessibility')}
                className={`text-xs font-semibold px-4 py-2 rounded-full border border-white/20 transition-all ${granted.accessibility ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' : 'hover:bg-white/5 text-on-surface'}`}
              >
                {granted.accessibility ? 'Access Granted' : 'Grant Access'}
              </button>
            </div>
            <div>
              <h3 className="text-lg font-bold text-on-surface mb-2">Accessibility Service</h3>
              <p className="text-xs text-on-surface-variant leading-relaxed">
          ...
          <div className="glass-panel rounded-2xl p-6 hover:-translate-y-1 transition-all flex flex-col justify-between h-full border border-white/5">
            <div className="flex justify-between items-start mb-6">
              <div className="bg-error-container/20 p-3 rounded-xl border border-error/20">
                <Mic className="w-6 h-6 text-red-400" />
              </div>
              <button
                onClick={() => handleToggle('microphone')}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${granted.microphone ? 'bg-cyan-500' : 'bg-surface-container-highest'}`}
              >
            ...
            <div>
              <h3 className="text-lg font-bold text-on-surface mb-2">Microphone</h3>
              <p className="text-xs text-on-surface-variant leading-relaxed">
        ...
        <div className="flex flex-col items-center mt-8 gap-4 max-w-md mx-auto w-full">
          <button
            ...
            className="w-full py-4 rounded-xl shadow-lg electric-flow text-white font-semibold flex items-center justify-center gap-2 hover:opacity-95 transition-opacity active:scale-95 shadow-cyan-500/20"
          >
